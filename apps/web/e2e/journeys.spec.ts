...
for (const devNoAuth of [false, true]) {
  test(`project, graph, workflow, history and restart (${devNoAuth ? "no token" : "authenticated"})`, async ({
    page,
  }) => {
    ...
    git("checkout", "-b", "feat/graph");
    git("commit", "--allow-empty", "-m", "커밋 그래프 분기 확인");
    git("checkout", "main");
    git("commit", "--allow-empty", "-m", "Main progress");
    git("merge", "--no-ff", "feat/graph", "-m", "Merge graph feature");
    const stop = async () => {
    ...
    const start = async () => {
      child = spawn(resolve("../../artifacts/dev/otterd"), [], {
        env: {
          ...process.env,
          OTTER_AUTO_IMPORT: "0",
          OTTER_DEV_NO_AUTH: devNoAuth ? "1" : "0",
    ...
    try {
      ...
      if (devNoAuth) {
        await expect(
          page.getByText("Dev · no token", { exact: true }),
        ).toBeVisible();
        expect(
          await page.evaluate(() => sessionStorage.getItem("otter-token")),
        ).toBeNull();
      }
      ...
      await expect(
        page.getByRole("heading", { name: "Commit tree", exact: true }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: /Merge graph feature/ }),
      ).toBeVisible();
      await page.getByRole("button", { name: /커밋 그래프 분기 확인/ }).click();
      await expect(page.locator(".commit-detail")).toContainText("feat/graph");
      expect(
        (await page.locator(".workspace").boundingBox())!.height,
      ).toBeLessThan(220);
      await page.screenshot({
        path: resolve("../../artifacts/screenshots/project-overview.png"),
        fullPage: true,
      });
      await page
        ...
        .click();
      await expect(
        page.getByRole("heading", { name: "Registered projects (1)" }),
      ).toBeVisible();
      ...
      await page
        .getByLabel("Message recipient", { exact: true })
        .selectOption("Reviewer");
      await page
        .getByLabel("Request", { exact: true })
        .fill("Keep this unsent task draft");
      ...
      await expect(
        page.getByLabel("Message recipient", { exact: true }),
      ...
      await page
        .locator("main")
        .getByRole("button", { name: /River parser recovery/ })
        .click();
      await expect(
        page.getByRole("button", { name: "Workflow complete", exact: true }),
      ).toBeDisabled();
      ...
      await page.getByLabel("JSONL path on daemon machine").fill(codexSession);
      await page
      ...
      await page
        .getByRole("button", { name: "◷ Sessions", exact: true })
        .click();
      const subject = page.locator(".session-subject strong", {
        hasText: "파서 경계 처리 개선",
      });
      ...
      await page
        .getByLabel("Request", { exact: true })
        .fill("Inspect using the selected recipient");
      await page
        .getByRole("button", { name: "Send request", exact: true })
        .click();
      await reviewer.getByRole("button", { name: "Stop", exact: true }).click();
      ...
      await page
        .getByRole("button", { name: "Remove repo from Otter", exact: true })
        .click();
      await expect(
        page.getByText(/Only the registration is removed/),
      ).toBeVisible();
      ...
      const worktreesBeforeRemoval = git(
        "worktree",
        "list",
        "--porcelain",
      ).toString();
      ...
      await expect(page.getByRole("status")).toContainText(
        "Files and history were kept",
      );
      await expect(
        page.getByRole("heading", { name: "Registered projects (0)" }),
      ).toBeVisible();
      expect(git("rev-parse", "HEAD").toString()).toBe(headBeforeRemoval);
      expect(git("worktree", "list", "--porcelain").toString()).toBe(
        worktreesBeforeRemoval,
      );
      expect((await apiGet(`/work-units/${unit.id}`)).work_unit.id).toBe(
        unit.id,
      );
      await page.reload();
      await expect(
