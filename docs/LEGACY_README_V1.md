...
`127.0.0.1:4317`에서 실행하며 개발 모드에서는 토큰 없는 접속 URL을 출력합니다. 다른 포트는
...
개발 모드에서는 `http://127.0.0.1:4317/`로 바로 접속합니다. 인증 모드의 주소는
`http://127.0.0.1:4317/#token=...`이며 토큰은 데몬 재시작 때 바뀌므로 공유하지 마세요.
서버의 SSH 주소가 다르면 위 주소를 바꾸면 됩니다.
...
개발 실행(`./scripts/dev`)은 기본적으로 토큰 없이 접속합니다. 기존 실행 파일도
`OTTER_DEV_NO_AUTH=1`을 지정해 재시작하면 같은 모드로 사용할 수 있습니다.
주소는 `http://127.0.0.1:4317/`이며 화면에 `Dev · no token`이 표시됩니다.
localhost·동일 출처 제한은 유지하지만, **같은 머신의 다른 프로세스도 API를 사용할 수
있으므로 공유 서버나 외부 공개 포트에서는 사용하지 마세요.**
`OTTER_DEV_NO_AUTH=0 ./scripts/dev`로 인증을 다시 켤 수 있습니다.
일반 배포 실행은 이 옵션을 명시하지 않으면 기존 토큰 인증을 유지합니다.

현재 기능 범위를 유지한 UI/UX 개선 내역과 VS Code·IntelliJ·Orca의 동선 비교는
[UX 검토](docs/UX_REVIEW.md)에 정리했습니다. 폴더 열기, 프로젝트 작업 바로가기,
목록을 유지하는 세션 상세, Work/History/Review 구분, 위치·임시 지시문 복원,
터미널 숨기기/종료 구분을 제공합니다. 터미널은 페이지를 새로고침하면 종료됩니다.
